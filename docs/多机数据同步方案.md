...
`engine/*.py`、`scripts/*.py`），默认 `postgresql://kc@localhost:5432/rating_system_national`。
...
```bash
# 每台机器 .env
DATABASE_URL=postgresql://rating:<强密码>@<虚拟内网IP>:5432/rating_system_national?sslmode=require

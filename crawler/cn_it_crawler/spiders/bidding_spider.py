...
策略:
1. 使用 WebSearchEngine 搜索 "{地域前缀 + }行业词 招标/采购" 等关键词
...
class BiddingSpider(scrapy.Spider):
    ...
    # 行业词 — 地域前缀由 config.yaml 的 region 段注入 (全国模式即纯行业词)
    INDUSTRY_TERMS = ['信息化', '数字化', 'AI', '人工智能',
                      '云计算', '大数据', '智慧城市', '软件开发']

    # 全国性招投标平台 — 始终保留
    NATIONAL_PLATFORM_DOMAINS = [
        'ccgp.gov.cn',            # 中国政府采购网
    ]
    ...
    def __init__(self, keyword=None, *args, **kwargs):
        ...
        # 地域前缀 + 行业词 → 搜索关键词 (全国模式为纯行业词)
        self.search_keywords = self.region.bidding_keywords(self.INDUSTRY_TERMS)
        # 本地平台域名 (config.yaml region.local_domains) + 全国平台
        self.platform_domains = list(dict.fromkeys(
            self.NATIONAL_PLATFORM_DOMAINS + self.region.local_domains))
        ...
        for kw in keywords:
            ...
            try:
                ...
                for result in results:
                    ...
                    if any(d in url.lower() for d in self.platform_domains):

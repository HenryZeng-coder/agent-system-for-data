...
class BusinessSpider(scrapy.Spider):
    """全国企业工商信息采集 Spider — 搜索引擎聚合"""

    name = 'business'
    allowed_domains = []

    # ---- 搜索关键词 ----
    # 行业词固定; 地域前缀来自 config.yaml 的 region 段 (全国模式无前缀)
    ...
    def __init__(self, mode='incremental', keyword=None, *args, **kwargs):
        ...
        if self.keyword_override:
            keywords = [self.region.query(self.keyword_override)]
        else:
            # 多查询变体: 提升字段提取率 (信用代码/成立日期/法人/地址)
            base_keywords = [self.region.query(ind) for ind in self.industries]
            query_variants = [
    ...
    def _extract_company_name(self, text, keyword):
        ...
        # 匹配中文公司名
        patterns = [
            self.region.company_name_pattern(),
    ...
    def _extract_address(self, text):
        """提取注册地址 — 优先匹配 注册地址/住所 前缀, 回退到 region 地址模式"""
        m = re.search(r'(?:注册地址|住所|地址)[：:]\s*([^\s,，。]{6,80})', text)
        if m:
            addr = m.group(1).strip()
            if self.region.looks_like_address(addr):
                return addr
        # 回退: 匹配 "{地域}...号/路/大道/街/区" 形式的地址片段
        m = re.search(self.region.address_pattern(), text)
        return m.group(1) if m else None

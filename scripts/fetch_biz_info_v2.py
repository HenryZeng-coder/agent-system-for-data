#!/usr/bin/env python3
"""第二渠道补全: 必应搜索定位真实注册名 → 爱企查/水滴信用详情页 → 提取信用代码

用法: python scripts/fetch_biz_info_v2.py [--in PATH] [--out PATH]

对 fetch_biz_info.py 未命中的企业, 用必应搜索:
1. 搜索 \"企业名\" 找到真实注册名 (简称 → 全称, 如 \"某某科技\" → \"XX科技(省)有限公司\")
...
DEFAULT_IN = os.path.join(POOL_DIR, "biz_info.csv")
DEFAULT_OUT = os.path.join(POOL_DIR, "biz_info_v2.csv")
...
def main():
    ...
    parser.add_argument("--out", dest="out_path", default=DEFAULT_OUT, help="结果输出 CSV")

...
from cn_it_crawler.pipelines.filter_pipeline import FilterPipeline
from cn_it_crawler.pipelines.clean_pipeline import CleanPipeline
from cn_it_crawler.pipelines.validate_pipeline import ValidatePipeline
from cn_it_crawler.pipelines.standardize_pipeline import StandardizePipeline
from cn_it_crawler.items import CompanyItem
...
class TestCleanPipeline:
    ...
    def test_html_removal(self):
        """清除 HTML 标签"""
        p = CleanPipeline()
        item = self._make_item(company_name="<b>示例</b>科技")
    ...
    def test_whitespace_compression(self):
        """压缩多余空白"""
        p = CleanPipeline()
        item = self._make_item(company_name="示例  科技  公司")
        result = p.process_item(item, None)
        assert result["company_name"] == "示例 科技 公司"
...
class TestValidatePipeline:
    ...
    def test_valid_item_pass(self):
        """company_name 非空 → 保留"""
        p = ValidatePipeline()
        item = self._make_item(company_name="示例AI", credit_code="91420100MA4K00001")
        assert p.process_item(item, None) is item
    ...
    def test_noise_name_dropped(self):
        ...
        p = ValidatePipeline()
        item = self._make_item(company_name="搜索示例AI结果")
        try:
            p.process_item(item, None)
            assert False, "应该抛出 DropItem"
        except DropItem:
    ...
    def test_news_future_date_nullified(self):
        """新闻发布日期晚于今天 → 置空"""
        from cn_it_crawler.items import NewsMentionItem
        p = ValidatePipeline()
    ...
    def test_budget_negative_nullified(self):
        """招标预算为负 → 置空"""
        from cn_it_crawler.items import BiddingItem
        p = ValidatePipeline()
        item = BiddingItem()
        item["company_id"] = 1
